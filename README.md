...
Docker container with Apache PHP 4.4 build by [Nix package manager](https://nixos.org/).
...
## Building Apache PHP 4.4

To build Apache PHP 4.4, follow these steps:
...
[![Build Status](https://jenkins.majordomo.ru/job/webservices/job/apache2-php44/job/master/lastBuild/badge/icon?config=webservices-apache2-php44)](https://jenkins.majordomo.ru/job/webservices/job/apache2-php44/job/master/lastBuild/badge/icon?config=webservices-apache2-php44)

## Using Apache PHP 4.4

To use Apache PHP 4.4, call:

``` shell
docker inspect docker-registry.intr/webservices/apache2-php44 | grep cmd
```
...
docker run --read-only --network=host \
        ...
        --ulimit stack=-1:-1 \
        --cap-add SYS_ADMIN \
        --security-opt apparmor:unconfined docker-registry.intr/webservices/apache2-php44:latest
```
...
* `$SOCKET_HTTP_PORT` any free port on your computer, e.g. `8044`.

## Contributing to Apache PHP 4.4

To contribute to Apache PHP 4.4, follow these steps:
